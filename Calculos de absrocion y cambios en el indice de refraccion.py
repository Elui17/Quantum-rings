...
miu0 = 4 * np.pi * 10**-7  # Permeabilidad magnética, en H/m
...
for indice in indice_values:
    ...
    def term1(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M12_SOI_NoImp[indice]  * q**2) / (hbar * ((E12_SOI_NoImp[indice] * q - (ei * q/1000 ))**2 + gamma12**2)))
        
    def term2(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M13_SOI_NoImp[indice]  * q**2) / (hbar * ((E13_SOI_NoImp[indice] * q - (ei * q /1000))**2 + gamma12**2)))

    def term3(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M14_SOI_NoImp[indice]  * q**2) / (hbar * ((E14_SOI_NoImp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))

    def term4(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M15_SOI_NoImp[indice]  * q**2) / (hbar * ((E15_SOI_NoImp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))

    def term5(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M16_SOI_NoImp[indice]  * q**2) / (hbar * ((E16_SOI_NoImp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))
   ...
   # Guardar la tabla de datos como CSV
    filename_csv = f"SOI_Sin_impureza_B_{resultado[0]}_P_{resultado[1]}_T_{resultado[2]}.csv"
    ...
    def term11(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M12_NoSOI_NoImp[indice]  * q**2) / (hbar * ((E12_NoSOI_NoImp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))
        
    def term22(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M13_NoSOI_NoImp[indice]  * q**2) / (hbar * ((E13_NoSOI_NoImp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))

    def term33(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M14_NoSOI_NoImp[indice]  * q**2) / (hbar * ((E14_NoSOI_NoImp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))
   ...
   # Guardar la tabla de datos como CSV
    filename_csv1 = f"NOSOI_Sin_impureza_B_{resultado[0]}_P_{resultado[1]}_T_{resultado[2]}.csv"
    ...
    plt.ylabel("Coef. Absor ($10^{5}m^{-1}$) ", fontweight='bold')
    ...
    filename = f"Sin_impureza_B_{resultado[0]}_P_{resultado[1]}_T_{resultado[2]}.png"
    ...
    def term111(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M12_SOI_Imp[indice]  * q**2) / (hbar * ((E12_SOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))
        
    def term222(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M13_SOI_Imp[indice]  * q**2) / (hbar * ((E13_SOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))

    def term333(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M14_SOI_Imp[indice]  * q**2) / (hbar * ((E14_SOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))

    def term444(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M15_SOI_Imp[indice]  * q**2) / (hbar * ((E15_SOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))

    def term555(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M16_SOI_Imp[indice]  * q**2) / (hbar * ((E16_SOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))
   ...
   # Guardar la tabla de datos como CSV
    filename_csv2 = f"SOI_impureza_B_{resultado[0]}_P_{resultado[1]}_T_{resultado[2]}.csv"
    ...
    def term1111(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M12_NoSOI_Imp[indice]  * q**2) / (hbar * ((E12_NoSOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))
        
    def term2222(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M13_NoSOI_Imp[indice]  * q**2) / (hbar * ((E13_NoSOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))

    def term3333(ei):
        return (1/10**5)*(ei * q / 1000) * np.sqrt(miu0 / er_function(T_value, P_value)) * ((sigma_v * gamma12 * M14_NoSOI_Imp[indice]  * q**2) / (hbar * ((E14_NoSOI_Imp[indice] * q - (ei * q / 1000))**2 + gamma12**2)))
   ...
   # Guardar la tabla de datos como CSV
    filename_csv3 = f"NOSOI_impureza_B_{resultado[0]}_P_{resultado[1]}_T_{resultado[2]}.csv"
    ...
    plt.grid(True)
    # Guardar la gráfica con un nombre que incluya los parámetros
    filename1 = f"impureza_B_{resultado[0]}_P_{resultado[1]}_T_{resultado[2]}.png"
